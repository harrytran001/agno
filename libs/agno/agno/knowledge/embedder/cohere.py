...
class CohereEmbedder(Embedder):
    ...
    def _rate_limit_backoff_sleep(self, attempt: int) -> None:
        """
        Sleep with rate-limit-aware backoff for APIs with per-minute limits.
        
        For 40 req/min APIs like Cohere Trial:
        - Attempt 1: 15s wait (total: 15s)
        - Attempt 2: 30s wait (total: 45s) 
        - Attempt 3: 60s wait (total: 105s)
        This gives the API time to reset its rate limit window.
        """
        if attempt == 0:
            delay = 15  # Wait 15 seconds (1/4 of minute window)
        elif attempt == 1:
            delay = 30  # Wait 30 seconds (1/2 of minute window)  
        else:
            delay = 60  # Wait full minute for window reset
            
        # Add small jitter
        delay += (time.time() % 3)
        
        log_debug(f"Rate limit backoff, waiting {delay:.1f} seconds for rate limit window reset (attempt {attempt + 1})")
        time.sleep(delay)
    ...
    async def _async_rate_limit_backoff_sleep(self, attempt: int) -> None:
        ...
        log_debug(f"Async rate limit backoff, waiting {delay:.1f} seconds for rate limit window reset (attempt {attempt + 1})")
        await asyncio.sleep(delay)

    def _batch_with_retry(self, texts: List[str], max_retries: int = 3) -> Tuple[List[List[float]], List[Optional[Dict]]]:
        """Execute batch embedding with rate-limit-aware backoff for rate limiting."""
        log_debug(f"Starting batch retry for {len(texts)} texts with max_retries={max_retries}")
        
        for attempt in range(max_retries + 1):
            try:
                request_params = self._get_batch_request_params()
                response: Union[EmbeddingsFloatsEmbedResponse, EmbeddingsByTypeEmbedResponse] = self.client.embed(
                    texts=texts, **request_params
                )
                ...
                log_debug(f"Batch embedding succeeded on attempt {attempt + 1}")
                return batch_embeddings, all_usage
                
            except Exception as e:
                if self._is_rate_limit_error(e):
                    if not self.enable_backoff:
                        log_warning(f"Rate limit detected. To enable automatic backoff retry, set enable_backoff=True when creating the embedder.")
                        raise e
                    
                    log_info(f"Rate limit detected on attempt {attempt + 1}")
                    if attempt < max_retries:
                        self._rate_limit_backoff_sleep(attempt)
                        continue
                    else:
                        log_warning(f"Max retries ({max_retries}) reached for rate limiting")
                        raise e
                else:
                    log_debug(f"Non-rate-limit error on attempt {attempt + 1}: {e}")
                    raise e
        
        # This should never be reached, but just in case
        log_error("Unexpected: Reached end of retry loop")
        return [], []

    async def _async_batch_with_retry(self, texts: List[str], max_retries: int = 3) -> Tuple[List[List[float]], List[Optional[Dict]]]:
        ...
        # This should never be reached, but just in case
        log_error("Async unexpected: Reached end of retry loop")
        return [], []
    ...
    def get_embeddings_batch_and_usage(self, texts: List[str], batch_size: int = 100) -> Tuple[List[List[float]], List[Optional[Dict]]]:
        ...
        all_embeddings = []
        all_usage = []
        log_info(f"Getting embeddings and usage for {len(texts)} texts in batches of {batch_size}")
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                batch_embeddings, batch_usage = self._batch_with_retry(batch_texts)
                ...
                if self._is_rate_limit_error(e):
                    ...
                    if len(batch_texts) > 1:
                        ...
                        for j in range(0, len(batch_texts), smaller_batch_size):
                            ...
                            try:
                                small_embeddings, small_usage = self._batch_with_retry(small_batch)
                    ...
                    for text in batch_texts:
                        try:
                            ...
                            log_warning(f"Error in individual embedding fallback: {e2}")
                            all_embeddings.append([])
                            all_usage.append(None)
    ...
    async def async_get_embeddings_batch_and_usage(self, texts: List[str], batch_size: int = 100) -> Tuple[List[List[float]], List[Optional[Dict]]]:
        """
...
s, List of usage dictionaries)
        ...
        all_embeddings = []
        all_usage = []
        log_info(f"Getting embeddings and usage for {len(texts)} texts in batches of {batch_size} (async)")
